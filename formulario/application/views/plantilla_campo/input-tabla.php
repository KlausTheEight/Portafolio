			...
			<tr>
				...
				for ($j = 0; $j < count($configuracion["titulos"][0]); $j++) {
					...
					<?php }else{
						...
						?>
						<td>
							<input <?= $readonly ?> min="0" class="tabla_<?php $id ?>" style="width: 100%;" type="<?= $tipo ?>"
							<?php if ($readonly == ""): ?>
								name="campo_<?php echo $id ?><?= "[".$j."][".$i."]" ?>"
							<?php endif ?>
							value="<?= $valores != null && isset($valores[$j]) && isset($valores[$j][$i]) ? $valores[$j][$i] : "" ?>">
						</td>
						<?php
					} ?>
					<?php
